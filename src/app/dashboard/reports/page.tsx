  ...
  return (
    <div className="container">
      <header style={{ marginBottom: '4rem', display: 'flex', alignItems: 'center', gap: '2rem' }}>
        ...
        <div>
          <span className="badge">DEDICATED AI AGENTS</span>
          <h1 className="glitter-text" style={{ fontSize: '3.5rem', margin: 0 }}>Clinical Workspace</h1>
        </div>
      </header>
      ...
      <div className="reports-layout" style={{ display: 'grid', gridTemplateColumns: 'minmax(300px, 350px) 1fr', gap: '3rem' }}>
        <aside style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
           {[
            { id: 'Medical Report', icon: <Stethoscope size={28} />, label: 'Standard Agent', color: '#3b82f6' },
            { id: 'Blood Test', icon: <Droplets size={28} />, label: 'Blood Agent', color: '#ef4444' },
            { id: 'Medication', icon: <Pill size={28} />, label: 'Pharma Agent', color: '#10b981' }
          ].map((tool) => (
            <div 
              key={tool.id} 
              className="card" 
              style={{ cursor: 'pointer', padding: '2rem', border: activeAnalyzer === tool.id ? '4px solid #3b82f6' : '1px solid var(--border)' }}
              onClick={() => { setActiveAnalyzer(tool.id); setAnalysisResult(null); setErrorStatus(null); }}
            >
        ...
        <main className="card" style={{ padding: '3rem' }}>
          <AnimatePresence mode="wait">
            {!activeAnalyzer ? (
              <div style={{ textAlign: 'center', padding: '10vw 0' }}><Search size={64} color="#e5e7eb" /><h2 style={{ color: '#94a3b8', marginTop: '2rem' }}>Awaiting Agent Selection...</h2></div>
            ) : !analysisResult && !isAnalyzing ? (
               <div className="upload-zone" style={{ border: '2px dashed #cbd5e1', borderRadius: '2rem', padding: '5rem 1rem', textAlign: 'center' }} onClick={() => fileInputRef.current?.click()}>
                  {errorStatus && (
                    <div style={{ background: '#fee2e2', color: '#991b1b', padding: '1rem', borderRadius: '1rem', marginBottom: '2rem' }}>
                      <AlertCircle style={{ display: 'inline', marginRight: '8px' }} size={16} /> ERROR: {errorStatus}
                    </div>
                  )}
                  <ScanSearch size={64} color="#3b82f6" />
                  <h2 style={{ fontSize: '2.2rem', marginTop: '2rem' }}>Inject {activeAnalyzer} Data</h2>
                  <p style={{ color: 'var(--muted)', marginTop: '1rem' }}>PDF/XLS/IMG Supported • Advanced Compression Enabled</p>
                  <button className="primary" style={{ margin: '2rem auto 0' }}>LAUNCH UPLOAD</button>
              ...
              <div style={{ textAlign: 'center', padding: '8rem 0' }}>
                 ...
                 <h2 style={{ fontSize: '4rem', fontWeight: 950, color: '#3b82f6', marginTop: '2rem' }}>{progress}%</h2>
                 ...
                 <div style={{ width: '100%', maxWidth: '400px', height: '12px', background: '#f1f5f9', borderRadius: '6px', margin: '3rem auto', overflow: 'hidden' }}>
                    <motion.div animate={{ width: `${progress}%` }} style={{ height: '100%', background: 'linear-gradient(90deg, #3b82f6, #d946ef)' }} />
                 </div>
              </div>
            ) : (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                 <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', gap: '2rem', marginBottom: '4rem' }}>
                    ...
                    <button className="secondary" style={{ width: 'auto' }} onClick={() => setAnalysisResult(null)}>RESET AGENT</button>
                 </div>
                 ...
                 <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '2.5rem', marginBottom: '4rem' }}>
                    <div className="card" style={{ background: '#ecfdf5', borderColor: 'transparent' }}>
                       ...
                       <ul style={{ paddingLeft: '1.25rem', color: '#065f46' }}>{analysisResult.positives?.map((p: string, i: number) => <li key={i} style={{ marginBottom: '1rem', fontWeight: 600 }}>{p}</li>)}</ul>
                    </div>
                    ...
                    <div className="card" style={{ background: '#fffbeb', borderColor: 'transparent' }}>
                       ...
                       <ul style={{ paddingLeft: '1.25rem', color: '#92400e' }}>{analysisResult.improvements?.map((p: string, i: number) => <li key={i} style={{ marginBottom: '1rem', fontWeight: 600 }}>{p}</li>)}</ul>
                    </div>
                    ...
                    <div className="card" style={{ background: '#fef2f2', borderColor: 'transparent' }}>
                       ...
                       <ul style={{ paddingLeft: '1.25rem', color: '#991b1b' }}>{analysisResult.risks?.map((p: string, i: number) => <li key={i} style={{ marginBottom: '1rem', fontWeight: 600 }}>{p}</li>)}</ul>
                    </div>
                 </div>

                 <div className="card" style={{ background: '#020617', color: 'white', padding: '4rem' }}>
                    <h3 style={{ color: '#3b82f6', marginBottom: '2.5rem', fontSize: '2rem', fontWeight: 950 }}>Agent Interpretation</h3>
