    ...
    <div className="container" style={{ background: '#fdfdfd', minHeight: '100vh' }}>
      <header style={{ marginBottom: '4rem', display: 'flex', alignItems: 'center', gap: '2rem' }}>
        ...
        <div>
          ...
          <h1 className="glitter-text" style={{ fontSize: '3.5rem' }}>Report Analysis</h1>
        </div>
      </header>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '350px 1fr', gap: '3rem' }}>
        ...
        <main className="card" style={{ padding: '4rem' }}>
          <AnimatePresence mode="wait">
            {!activeAnalyzer ? (
              <div style={{ textAlign: 'center', padding: '8rem 0' }}><Search size={64} color="#e5e7eb" /><h2 style={{ color: '#94a3b8', marginTop: '2rem' }}>Select what you want to analyse...</h2></div>
            ) : !analysisResult && !isAnalyzing ? (
              <div className="upload-zone" onClick={() => fileInputRef.current?.click()}>
                 <ScanSearch size={64} color="#3b82f6" />
                 <h2 style={{ fontSize: '2.5rem', marginTop: '2rem' }}>Upload {activeAnalyzer}</h2>
                 <p style={{ color: 'var(--muted)', marginTop: '1rem' }}>Upload PDF, Images, Word, or Excel</p>
              ...
              <div style={{ textAlign: 'center', padding: '8rem 0' }}>
                 ...
                 <p style={{ color: 'var(--muted)', fontSize: '1.2rem', marginTop: '1rem' }}>Analysing {activeAnalyzer}...</p>
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                 <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4rem' }}>
                    <div>
                      <h2 style={{ fontSize: '3rem' }} className="glitter-text">{analysisResult.title || "Analysis Result"}</h2>
                      <p style={{ color: 'var(--muted)', fontSize: '1.2rem' }}>{analysisResult.summary}</p>
                    </div>
                    <button className="secondary" onClick={() => setAnalysisResult(null)}>Analyses New Report</button>
                 </div>
                 ...
                 <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '2rem', marginBottom: '4rem' }}>
                    <div className="card" style={{ background: '#ecfdf5', borderColor: '#34d399' }}>
                       ...
                       <ul style={{ paddingLeft: '1.25rem' }}>{analysisResult.positives?.map((p: string, i: number) => <li key={i} style={{ marginBottom: '0.75rem', fontSize: '0.95rem' }}>{p}</li>)}</ul>
                    </div>
                    <div className="card" style={{ background: '#fffbeb', borderColor: '#fbbf24' }}>
                       <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1.5rem', color: '#d97706' }}>
                          <Zap size={24} /> <h3 style={{ color: '#d97706' }}>Needs Improvement</h3>
                       </div>
                       <ul style={{ paddingLeft: '1.25rem' }}>{analysisResult.improvements?.map((p: string, i: number) => <li key={i} style={{ marginBottom: '0.75rem', fontSize: '0.95rem' }}>{p}</li>)}</ul>
                    </div>
                    <div className="card" style={{ background: '#fef2f2', borderColor: '#f87171' }}>
                       <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1.5rem', color: '#dc2626' }}>
                          <AlertCircle size={24} /> <h3 style={{ color: '#dc2626' }}>Potential Risks</h3>
                       </div>
                       <ul style={{ paddingLeft: '1.25rem' }}>{analysisResult.risks?.map((p: string, i: number) => <li key={i} style={{ marginBottom: '0.75rem', fontSize: '0.95rem' }}>{p}</li>)}</ul>
                    </div>
                 </div>

                 <div className="card" style={{ background: '#020617', color: 'white', padding: '4rem' }}>
                    <h3 style={{ color: '#3b82f6', marginBottom: '2rem', fontSize: '1.8rem' }}>AI Interpretation</h3>
