      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '3.5rem' }}>
        ...
        <section className="card" style={{ padding: '0' }}>
           ...
           <div style={{ padding: '4rem', textAlign: 'center' }}>
              ...
              <p style={{ color: 'var(--muted)', marginTop: '0.5rem' }}>Molecular nutrient identification agent</p>
           ...
           {isAnalyzingFood && (
            <div style={{ textAlign: 'center', paddingBottom: '6rem' }}>
               ...
               <h3 style={{ marginTop: '2.5rem', fontWeight: 900 }}>MAPPING MACRO MATRIX...</h3>
            </div>
           )}
             ...
             <div style={{ padding: '0 4rem 4rem' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '2rem' }}>
                   <div className="card" style={{ padding: '1.5rem', textAlign: 'center', background: '#f8fafc' }}>
                      <p style={{ fontSize: '0.7rem', color: 'var(--muted)' }}>ENERGY_DENSITY</p>
                      <p style={{ fontSize: '1.8rem', fontWeight: 950 }}>{calculateTotal('calories')} kcal</p>
                   </div>
                   <div className="card" style={{ padding: '1.5rem', textAlign: 'center', border: '1px solid #3b82f630' }}>
                      <p style={{ fontSize: '0.7rem', color: '#3b82f6' }}>AMINO_BASE</p>
                      <p style={{ fontSize: '1.8rem', fontWeight: 950 }}>{calculateTotal('protein')}g</p>
                   </div>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                   {foodResult.ingredients.map((ing: any, i: number) => (
                     <div key={i} className="card" style={{ padding: '1.5rem', background: 'white' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                           <h4 style={{ fontWeight: 900 }}>{ing.name}</h4>
                           <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', background: '#f1f5f9', padding: '0.4rem 0.8rem', borderRadius: '2rem' }}>
                              <button onClick={() => setServingSizes({...servingSizes, [i]: Math.max(0, servingSizes[i] - 10)})}><Minus size={14} /></button>
                              <span style={{ fontWeight: 950, fontSize: '0.9rem' }}>{servingSizes[i]}g</span>
                ...
                <button className="secondary" style={{ width: '100%', marginTop: '2.5rem' }} onClick={() => setFoodResult(null)}><X size={16} /> RESET SCANNER</button>
             </div>
           )}
        </section>

        {/* Bio-Kinetic Architect Card */}
        <section className="card" style={{ padding: '4rem' }}>
           <div style={{ textAlign: 'center', marginBottom: '4rem' }}>
              ...
              <p style={{ color: 'var(--muted)', marginTop: '0.5rem' }}>Goal-based energy optimization strategy</p>
           </div>
             ...
             <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                <div style={{ display: 'grid', gap: '2rem' }}>
                   {dietPlan.dietPlan.map((item: any, i: number) => (
                     <div key={i} className="card" style={{ padding: '2.5rem', background: 'white', border: editingMealIndex === i ? '2px solid #3b82f6' : '1px solid #e2e8f0' }}>
                        ...
                        {editingMealIndex === i ? (
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                             ...
                             <textarea value={item.description} style={{ height: '100px' }} onChange={e => {
                                const newP = {...dietPlan};
                          ...
                          <>
                             <h3 style={{ fontSize: '1.8rem', fontWeight: 950 }}>{item.meal}</h3>
                             <p style={{ color: 'var(--muted)', marginTop: '1rem', lineHeight: 1.8 }}>{item.description}</p>
                          </>
                        )}
                     </div>
                   ))}
